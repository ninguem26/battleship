...
/**
 * Classe com métodos para tratamentos de inputs, como validação de endereços
 * IPs e de entradas específicas do jogo.
 **/
public class InputHandler {
	...
	private static boolean validateAddShip(final String[] commands) {
		if (commands.length > 4) {
			...
			if (ship.length() > 1 || (ship.charAt(0) != 'S' && ship.charAt(0) != 'P' && ship.charAt(0) != 'C'
					&& ship.charAt(0) != 'N')) {
